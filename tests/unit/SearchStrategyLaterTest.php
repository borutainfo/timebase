...
/**
 * Class SearchStrategyLaterTest
 * @package Test
 */
final class SearchStrategyLaterTest extends AbstractTimebaseTest
{
    /**
     * @throws Exception
     */
    public function testSuccessfulExact(): void
    {
        for ($i = 0; $i < 100; $i++) {
            ...
            for ($j = 0; $j <= $i; $j++) {
                $result = $this->timebase->search()->storage(['test' . $i])->timestamp($this->timestamp + $j)->strategy(SearchStrategyConstant::LATER)->execute();
    ...
    public function testSuccessfulSingleFromSingle(): void
    {
        for ($i = 0; $i < 100; $i++) {
            ...
            for ($j = 0; $j <= $i; $j++) {
                $result = $this->timebase->search()->storage(['test' . $i])->timestamp($this->timestamp + $j * 2 - 1)->strategy(SearchStrategyConstant::LATER)->execute();
                self::assertNotEmpty($result);
                self::assertIsArray($result);
    ...
    public function testSuccessfulMultipleFromSingle(): void
    {
        for ($i = 0; $i < 100; $i++) {
            ...
            for ($j = 0; $j <= $i; $j++) {
                $result = $this->timebase->search()->storage(['test' . $i])->timestamp($this->timestamp + $j * 2 - 1)->strategy(SearchStrategyConstant::LATER)->all()->execute();
                self::assertNotEmpty($result);
                self::assertIsArray($result);
                self::assertEquals($result['timestamp'], $this->timestamp + $j * 2);
                self::assertEquals($result['value'], 'ts:' . ($this->timestamp + $j * 2));
    ...
    public function testSuccessfulFoundInLaterFile(): void
    {
        ...
        $result = $this->timebase->search()->storage(['test'])->timestamp($this->timestamp)->strategy(SearchStrategyConstant::LATER)->execute();
        self::assertNotEmpty($result);
        self::assertIsArray($result);
        ...
        $result = $this->timebase->search()->storage(['test'])->timestamp($this->timestamp)->strategy(SearchStrategyConstant::LATER)->all()->execute();
        self::assertNotEmpty($result);
        self::assertIsArray($result);
        self::assertEquals($result['timestamp'], $this->timestamp + 86400 * 4);
        self::assertEquals($result['value'], 'ts:' . ($this->timestamp + 86400 * 4));
    ...
    public function testUnsuccessfulNoDatabase(): void
    {
        ...
        $this->timebase->search()->storage(['test'])->timestamp($this->timestamp)->strategy(SearchStrategyConstant::LATER)->execute();
    }
}
